...
class Rss_controller extends CI_Controller {
	...
	public function section_year_sitemap() {
		...
		$min_year = $this->live_db->query("SELECT DATE_FORMAT((MIN(last_updated_on)), '%Y') as last_year from article")->row_array();
			...
			if($year  >= $min_year['last_year']) { 
				...
				$this->live_db->select("last_updated_on,url,content_id");
				...
				if($this->archive_db->table_exists($tablename."_".$year)) {
					...
					$this->archive_db->select("last_updated_on,url,content_id");
	...
	public function latest_sitemap(){
		...
		$SectionIds = array(23,24,21,22,19,20,27,28,29,31);
		$Response ='';
		for($i=0;$i<count($SectionIds);$i++):
			$GetArticle = $this->live_db->query("SELECT content_id,title,summary_html,article_page_content_html,publish_start_date,tags,last_updated_on,url,article_page_image_path FROM article WHERE section_id='".$SectionIds[$i]."' AND publish_start_date< NOW() AND status='P' ORDER BY last_updated_on DESC LIMIT 15"); 
			if($GetArticle->num_rows()!=0){
				foreach($GetArticle->result_array() as $ArticleDetails):
					...
					$content = str_replace(['alt=""' , '<p><img' , '/></p>' , '<p></p>' , '<p>&nbsp;</p>' ,'<p>  </p>' ,'<p> </p>' ,'<br>' ,'<h3> </h3>'] ,['' , '<figure><img' , '/></figure>' , '' ,'' ,'','','',''] , $content);
					...
					$EncodedContent 	.='</iframe></figure><footer><small>Copyright - '.date("Y").' edexlive.com. All rights reserved.</small></footer></article></body></html>]]>';
								...
								switch($widgetStyle){
									case 1:
										if($renderMode=='manual'){
											...
											$categoryId = (int) $widgets[$j]['cdata-widgetCategory'];
											if($categoryId==0){
												$articleDetailQuery = "SELECT content_id , section_id , url , section_name , title , author_name , publish_start_date , summary_html , agency_name , tags  , article_page_content_html as content , article_page_image_path as image , article_page_image_title as imagetitle , article_page_image_alt as imagealt FROM article WHERE status='P' AND publish_start_date < NOW() ORDER BY publish_start_date DESC LIMIT ".$maxArticles."";
												if(!$this->memcached_library->get($articleDetailQuery) && $this->memcached_library->get($articleDetailQuery) == ''){
													$articleDetail = $this->live_db->query($articleDetailQuery)->result_array();
													$this->memcached_library->add($articleDetailQuery,$articleDetail);
												}else{
													$articleDetail = $this->memcached_library->get($articleDetailQuery);
												}
												foreach($articleDetail as $articleList){
													$articleList['content_type_id'] = 1;
													array_push($list , $articleList);
												}
												//$articleDetail['content_type_id'] = 1;
