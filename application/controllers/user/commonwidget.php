...
class Commonwidget extends CI_Controller 
{
	...
	public function nextArticle(){
		...
		$ArticleCount = $this->input->post('articlecount');
		$CurrentArticleCount = $this->input->post('currentarticleid');
		if($contentType=='' || $articleId=='' || $sectionId=='' || $contentType!='1'):
			$errorResponse = [];
			$errorResponse['status'] = 0;
			echo json_encode($errorResponse);
		else:
			$response = $this->widget_model->getNextArticle($contentType , $articleId , $sectionId , $ArticleCount , $CurrentArticleCount);
...
if(count($widget_contents)>0)
		...
		{
		$show_simple_tab .= '<div>
					 <div class="state2"><a  href="'.$live_article_url.'" class="article_click" ><img src="'.$dummy_image.'"  data-src="'.$show_image.'" title = "'.$imagetitle.'" alt = "'.$imagealt.'"></a>';
	...
	if($count<=$menu_count && $show_image!='' && $parent_sectionname !='Columns/'){
	   ...
	   $show_simple_tab .='<div class="MultiImageContent">';
		...
		$show_simple_tab .='<img src="'.$show_image.'" data-src="'.$show_image.'"  title = "'.$imagetitle.'" alt = "'.$imagealt.'"></a>
	 '.$display_title.'</div>';
...
$show_recent_article = '<div id="slidebox" class="slide-box"> <i class="fa fa-times slide-close"></i>';
 $show_recent_article.='<h4 class="SiteColor article-recent">Recent '.$recent_text.' in '.$section_name.'</h4>';
  $show_recent_article.='<div class="slide-headlines"> <img src="'.$recent_article_img.'" data-src="'.$recent_article_img.'" title="'.$image_title.'" alt="'.$image_alt.'" />
...
$show_recent_article = '<div id="slidebox" class="slide-box"> <i class="fa fa-times slide-close"></i>';
 $show_recent_article.='<h4 class="SiteColor article-recent">Recent '.$recent_text.' in '.$recent_article['section_name'].'</h4>';
  $show_recent_article.='<div class="slide-headlines"> <img src="'.$recent_article_img.'" data-src="'.$recent_article_img.'" title="'.$img_title.'" alt="'.$img_alt.'" />
