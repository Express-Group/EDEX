...
if(isset($live_articles) && count($live_articles)) {
	...
	$mobileUrl = explode('/',html_entity_decode(@$articles['url'], null, "UTF-8"));
	$mUrl = 'https://m.edexlive.com/article/'.$mobileUrl[0].'/'.str_replace('-'.$articles['content_id'].'.html','',end($mobileUrl)).'/'.$articles['content_id'];
	?>
...
if(isset($new_articles) && count($new_articles)) {
    ...
    <news:news>
      <news:publication>
        <news:name>Edexlive</news:name>
        <news:language>en</news:language>
      </news:publication>
      <news:publication_date><?php echo gmdate('Y-m-d\TH:i:s+05:30',strtotime(@$articles['publish_start_date'])); ?></news:publication_date>
      <news:title><?php 
