...
class Widget_model extends CI_Model
{
  ...
  public function __construct() 
	...
	public function GetMetaData($value){ 
		$data = [];
		$Query=$this->db->query("SELECT meta_name,meta_description FROM tag_master WHERE tag_name='".trim($value)."'");
		if($Query->num_rows()==0){
			$data['name']='';
			$data['description']='';
		}else{
			$result = $Query->row_array();
			$data['name']=$result['meta_name'];
			$data['description']=$result['meta_description'];
		}
		return $data;
	}
	public function getNextArticle($contentType , $articleId , $sectionId ,$ArticleCount , $CurrentContentID){
		$class_object = new Widget_config_class;
		return $class_object->getNextArticle($contentType , $articleId , $sectionId ,$ArticleCount , $CurrentContentID);
	}
...
class Widget_config_class extends Widget_model
{
	...
	public function getNextArticle($contentType , $articleId , $sectionId ,$ArticleCount , $CurrentContentID){
		// status - 1 : hasArticle  || status - 0 : none
		$response = [];
		//$query=$this->live_db->query("SELECT content_id, title , url , summary_html ,article_page_content_html ,publish_start_date ,author_name ,author_image_path ,article_page_image_path ,article_page_image_title ,article_page_image_alt , tags FROM article WHERE section_id='".$sectionId."' AND content_id > '".$articleId."' AND status='P' LIMIT 1");
		if($ArticleCount == '1'){
			$query=$this->live_db->query("SELECT content_id, title , url , summary_html ,article_page_content_html ,publish_start_date ,author_name ,author_image_path ,article_page_image_path ,article_page_image_title ,article_page_image_alt , tags ,meta_Title FROM article WHERE  content_id != '".$CurrentContentID."' AND section_id='".$sectionId."'  AND status='P' ORDER BY content_id DESC LIMIT 1");
		}else{
			$query=$this->live_db->query("SELECT content_id, title , url , summary_html ,article_page_content_html ,publish_start_date ,author_name ,author_image_path ,article_page_image_path ,article_page_image_title ,article_page_image_alt , tags ,meta_Title FROM article WHERE  content_id != '".$CurrentContentID."' AND section_id='".$sectionId."'  AND status='P' AND  content_id < '".$articleId."'ORDER BY content_id DESC LIMIT 1");
		}
		if($query->num_rows()==0){
			$response['status'] = 0;
		}else{
			$result = $query->result();
			$response['status'] = 1;
			$response['contentid'] = $result[0]->content_id;
			$response['title'] = stripslashes(strip_tags($result[0]->title, '</p>'));
			$response['strippedtitle'] = stripslashes(strip_tags($result[0]->title));
			$response['summary'] = $result[0]->summary_html;
			$response['content'] = str_replace(image_url_no , image_url , $result[0]->article_page_content_html);
			$response['originalurl'] = $result[0]->url;
			$response['url'] =  base_url().$result[0]->url;
			$response['articleimage'] = $result[0]->article_page_image_path;
			$response['articleimagetitle'] = $result[0]->article_page_image_title;
			$response['articleimagetitlestripped'] = strip_tags($result[0]->article_page_image_title);
			$response['articleimagealt'] = $result[0]->article_page_image_alt;
			$response['authorname'] = $result[0]->author_name;
			$response['authorimage'] = $result[0]->author_image_path;
			$response['tags'] = $result[0]->tags;
			$response['metatitle'] = $result[0]->meta_Title;
			$response['publisheddate'] = date('dS  F Y ' , strtotime($result[0]->publish_start_date));
			$response['bittyurl']			= getSmallLink($response['url']); 
		}
		return $response;
	}
...
class Search extends Widget_model
{
    ...
    public function get_search_result_data($order_field, $order, $start, $length, $frm_search, $to_search, $search_value, $search_by, $section, $content_type, $type_search, $datafrom)
	{
		...
		$CacheID = 'CALL '.$search_procedure.'("'.$order_cond.'","'.$frm_search.'","'.$to_search.'","'.addslashes($search_value).'","'.$search_by.'","'.$section.'","'.$content_type.'", "'.$type_search.'")';
		if(!$this->memcached_library->get($CacheID) && $this->memcached_library->get($CacheID) == '') {
			...
			$this->memcached_library->add($CacheID,$live_result_manager);
			
		} else {
			 $live_result_manager = $this->memcached_library->get($CacheID);
		}
		//$live_result_manager =  $this->live_db->query('CALL '.$search_procedure.'("'.$order_cond.'","'.$frm_search.'","'.$to_search.'","'.addslashes($search_value).'","'.$search_by.'","'.$section.'","'.$content_type.'", "'.$type_search.'")')->result_array();	
